...
def read_dicom(path, label=False):
    ...
    if label:
        ...
        img = sitk.IntensityWindowing(img, -1024, 0, 0.0, 1.0)
...
trained_model = './lung_segment_model/model.cpkt'
...
def lung_segment(path, model_path):
    ...
    raw = sitk.IntensityWindowing(raw, -1024, 0, 0, 1.0)
...
def train(raw_path, label_path, model_path):
    ...
    path = trainer.train(data_provider, model_path, training_iters=64, epochs=200)
    print(path)
