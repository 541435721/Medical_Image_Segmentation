...
with g.as_default():
    ...
    with tf.variable_scope('predict'):
        out = tf.add_n(output)
        # pre = tf.nn.softmax(conv3D(out, 'output', [1, 1, 1, CLASSES, CLASSES], [1, 1, 1, 1, 1], 'SAME', 1.0))
    ...
    with tf.variable_scope('evaluate'):
        ...
        accs = []
        for PRE in output:
            pre_img_ = tf.argmax(PRE, -1)
            ans_ = tf.equal(pre_img_, Y)
            accs.append(tf.reduce_mean(tf.cast(ans_, tf.float32)))
            pre_imgs.append(pre_img_)
        ...
        while iteration < 100000:
            try:
                ...
                if portion < 0.4:
                    continue
                iteration += 1
                w = [portion, 1, 1.0 * (0.95 ** (iteration // 200)),  # [portion,1]
                ...
                ans1, ans2, ans3, ans4 = sess.run(
                    [loss, accs[0], rates, merged],
                    feed_dict={X: x, Y: y, W_class: w[:CLASSES], W_classifier: w[CLASSES:]})
                ...
                print(
                    "Iteration:{0},loss:{1},acc:{2},rates:{3},weight:{4}".format(str(iteration),
                                                                                 ans1,
                                                                                 ans2, ans3, w))
                pic = sess.run(pre_imgs, feed_dict={X: x, Y: y, W_class: w[:CLASSES], W_classifier: w[CLASSES:]})
                for k in range(BLOCK_SIZE[0]):
                    c = 1
                    for j in pic:
                        cv2.imwrite(
                            './prediction/pre_' + str(iteration) + "_" + str(k) + '_' + str(c) + '.jpg',
                            np.uint8(j[0, k, ...]) * 255)
                        c += 1
                        break
                        # cv2.imwrite(
