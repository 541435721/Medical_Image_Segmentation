...
if __name__ == '__main__':
    with tf.Session(graph=g) as sess:
        ...
        # saver.restore(sess, './test_model_save_norm/test.ckpt')
        tf.global_variables_initializer().run()
        key = 0.0045  # 0.005
        # sess.run(tf.assign(g_steps, 0))
        summary_writer = tf.summary.FileWriter('./summary_modified', graph=sess.graph)
        w = [0.1, 0.2, 0.3, 0.4]
        ans3 = 1000
        count = 0
        iteration = 0
        while iteration < 10000:
            try:
                ...
                cv2.imwrite(
                    './prediction_modified/pre_' + str(iteration) + '.jpg',
                    np.uint8(pic[0, 0, ...]) * 255)
            except Exception as e:
                print("出现异常，保存模型")
                saver.save(sess, './test_model_save_modified/test' + str(iteration) + '.ckpt')
        ...
        saver.save(sess, './test_model_save_modified/test.ckpt')
