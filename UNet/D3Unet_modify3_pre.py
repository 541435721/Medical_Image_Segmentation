...
def predict(vtk_file, model_path='./test_model_save/test.ckpt', out_file='out.vtk', blocksize=8):
    with tf.Session(graph=g) as sess:
        saver = tf.train.Saver()
        saver.restore(sess, model_path)
        Train_Data = read_dcm(vtk_file, True)
        prediction = np.zeros([1] + list(Train_Data.shape))
