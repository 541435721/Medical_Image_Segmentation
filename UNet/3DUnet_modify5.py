...
with g.as_default():
    ...
    pre_img_ = tf.cast(pre_img, tf.float32)
    ...
    DSC = 1.0 - (2 * tf.reduce_sum(pre_img_ * Y_) + 1) / (tf.reduce_sum(pre_img_) + tf.reduce_sum(Y_) + 1)

    loss = DSC + tf.add_n(tf.get_collection('l2_loss')) + tf.reduce_mean(weighted_loss)
...
if __name__ == '__main__':
    with tf.Session(graph=g) as sess:
        ...
        saver.restore(sess, './test_model_save_norm5/test.ckpt')
        key = 0.0045  # 0.005
        ...
        while iteration < 10000:
            try:
                ...
                if portion < 0.1:
                    continue
                iteration += 1
                w = [1, 1, 0.0001 * (0.90 ** (iteration // 200)),  # [portion,1]
                     0.0001 * (0.95 ** (iteration // 200))]

                ans1, ans2, ans3, ans4 = sess.run(
                    [DSC, acc, rates, merged], feed_dict={X: x, Y: y, W: w})
                ...
                for ii in range(BLOCK_SIZE[0]):
                    cv2.imwrite(
                        './prediction/pre_' + str(iteration) + '_' + str(ii) + '.jpg',
                        np.uint8(pic[0, ii, ...]) * 255)
            except Exception as e:
                print("出现异常，保存模型")
                # print(str(e))
                saver.save(sess, './test_model_save_5/test' + str(iteration) + '.ckpt')
        ...
        saver.save(sess, './test_model_save_5/test.ckpt')
