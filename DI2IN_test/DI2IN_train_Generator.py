...
with g.as_default():
    with tf.variable_scope('input'):
        X = tf.placeholder(dtype=tf.float32, shape=[1, None, None, None, 1])  # [batch,batchsize,w,h,c]
        Y = tf.placeholder(dtype=tf.float32, shape=[1, None, None, None, 1])  # [batch,batchsize,w,h]
    ...
    loss_g = tf.reduce_mean(
        tf.nn.sigmoid_cross_entropy_with_logits(labels=flatten_Y,
                                                logits=weighted_pre)) * W[2] + tf.nn.sigmoid_cross_entropy_with_logits(
        labels=1.0, logits=p2) * W[3]
    ...
    rates = tf.train.exponential_decay(0.02, g_steps, 200, 0.95, staircase=True)
    # train = tf.train.GradientDescentOptimizer(rates).minimize(loss, global_step=g_steps)
    train1 = tf.train.MomentumOptimizer(
        learning_rate=rates, momentum=0.2).minimize(loss=loss_g, global_step=g_steps, var_list=g_vars)
...
if __name__ == '__main__':
    with tf.Session(graph=g) as sess:
        ...
        saver.restore(sess, './test_model_save_good_D/test.ckpt')
        ...
        while iteration < 10000:
            try:
                ...
                if portion < 0.1 or list(x.shape) != [1] + [32, 256, 256] + [1]:
                    if list(x.shape) != [1] + [32, 256, 256] + [1]:
                        print(x.shape)
                    continue
                iteration += 1

                w = [1, portion, 1.8 * (0.99 ** (iteration // 200)),
                     1.0]
                y = y[..., np.newaxis]
                ...
                print(
                    "Iteration:{0},loss_g:{1},loss_d:{2},acc:{3},rates:{4},weight:{5}".format(str(iteration),
                                                                                              ...
                                                                                              ans4, ans3, w[0:2]))
                # P1, P2 = sess.run(
                #     [p1, p2], feed_dict={X: x, Y: y, W: w})
                # print(
                #     "Iteration:{0},loss_d:{1},label:{2},rates:{3}".format(str(iteration), ans2,
                #                                                           [P1, P2], ans3))
